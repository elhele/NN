...
inputSequence = transpose(0:2*pi/50:20*pi);
outputSequence = cos(inputSequence);
sequenceLength = length(outputSequence);
...
%%%% generate an esn 
nInputUnits = 1; nInternalUnits = 200; nOutputUnits = 1; 
% 
esn = generate_esn(nInputUnits, nInternalUnits, nOutputUnits, ...
    'spectralRadius',0.5,'inputScaling',[1],'inputShift',[0], ...
    'teacherScaling',[1],'teacherShift',[0],'feedbackScaling', 0, ...
    'type', 'plain_esn','learningMode','offline_singleTimeSeries'); 
...
%%%% train the ESN
nForgetPoints = 0 ; % discard the first 100 points
